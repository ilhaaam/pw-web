...
class ManagementController extends Controller
{
    ...
    {
        $path = settings( 'chat_log_path' ) . 'world2.chat';
        $num_lines = settings( 'chat_num_rows', 100 );
    ...
    public function postChatLogs()
    {
        ...
        $lines = [];
        $chatLine = $this->getChatLogs();
        
        foreach ($chatLine as $line) {
            $lines[] = explode(" ", $line);
        }
        foreach ( $lines as $o )
        {
            $chat[] = array(
                'row_color' => ( !array_key_exists( substr( $o[8], 4 ), $colors ) ) ? ( substr( $o[8], 4 ) <= 1024 ) ? 'font-faction' : 'font-whisper': $colors[ substr( $o[8], 4 ) ],
                'date' => $o[0]." ".$o[1],
                'type' => str_replace( ":", "", $o[6] ),
                'uid' => $this->getName( substr( $o[7], 4 ) ),
                'channel' => ( !array_key_exists( substr( $o[8], 4 ), $type ) ) ? ( substr( $o[8], 4 ) <= 1024 ) ? $type[3] : $type[4]: $type[ substr( $o[8], 4 ) ],
                'dest' => ( strpos( $o[8], 'fid' ) !== false ) ? $this->getFaction( substr( $o[8], 4 ) ) : $this->getName( substr( $o[8], 4 ) ),
                'msg' => base64_decode( base64_encode( iconv( "cp1252", "UTF-8", base64_decode( substr( $o[9], 4 ) ) ) ) )
            );
        }
    ...
    private function getName( $rid )
    {
        if ($rid >= 1024) {
            $name = Player::find($rid)['name'];
            return $rid . ' ( ' . $name . ' ) ';
        } else {
            return $rid;
        }
    ...
    private function getFaction( $fid )
    {
        $fname = Faction::find($fid)['name'];
        return $fid . ' ( ' . $fname . ' ) ';
    }
}
